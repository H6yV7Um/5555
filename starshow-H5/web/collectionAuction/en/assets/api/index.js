...
var initHomeDom=function(data){
	...
	expertsHtml+='<li>'+
					'<div class="expert_more">'+
						'<a href="expertLive.html?mt=2-0#experts">View&gt;<br/><span style="font-size:12px;">all experts</span></a>'+
					'</div>'+
				'</li>'
    ...
    for (var i = 0; i < collLenth; i++) {
	    	...
	    	if(experts.length<=0){
	    		appraisalsHtml+="<span class='no_data col_grey'>No one has ever written a comment</span>"
	    	}
    	
    		var str_status="";
	    	var status=collections[i].status;
	    	if(status==1){
	    		str_status='<span class="status">On View</span>'
	    	}else if(status==0){
	    		if(collections[i].auctions.length>0){
	    			str_status='<span class="status pre_status">start time：'+cutDate(collections[i].auctions[0].begin_time,"ymd")+'</span>'
	    		}else{
	    			str_status='<span class="status pre_status">Upcoming</span>'
	    		}
	    	 ...
	    	 collHtml+='<li><a href="goodsDetail.html?id='+collections[i].id+'&mt=1">'+
							...
							'<div class="auc_desc">'+
								...
								'<p>Seller：'+collections[i].user.name+'</p>'+str_status+
								'<div class="headList">'+
