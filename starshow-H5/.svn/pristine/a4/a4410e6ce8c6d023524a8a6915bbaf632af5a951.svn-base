   ...
   //活动分享
    var shareActivity=function(){
      setBridgeCallHandler(bridge, {
        'action': '3',
        'share': {
          'share_url':"https://lookmetv.com/starshow5.0/starZeroRange/share.html",
          'title': "与星零距离，邀你来现场",
          'content': "畅迷的年终福利，Show出你的爱，与偶像舒畅零距离",
          'cover': "https://starshow-pic.b0.upaiyun.com/default/icon/sc300.png"
        }
      })
    }
})
  ...
  var initCommentPage=function(){
      ...
      if($("#btn_down").html()!=undefined){
	      $(".btn_msgSend").click(function(){
	        ...
	        if(jwt_token=="" || jwt_token==undefined){
            //授权
             window.location.href = host+"wechat/activityAuth?type=lucky";
         }else{
	          toComment(jwt_token);
	        }
	      })
