...
function convertTextMsgToHtml(content, num_type) {
	...
	if (text.type != undefined) {
		...
		if (text.type == 0) {
			...
			if(text.bidder_name!=""){
				html += "<div class='bgRed'>presenter：The highest bidder is"+text.bidder_name+", If there is no bid, it will enter the auction countdown</div>";
			}else{
				html += "<div class='bgRed'>presenter：There is no bid at the moment, and the countdown is about to enter</div>";
			}
			$("#counter5s").show();
			...
			if(text.is_last_one==0){
				html += "<div class='bgRed'>presenter：Congratulations! “"+text.bidder_name+"”to get the collection and move on to the next one</div>";
			}else{
				html += "<div class='bgRed'>presenter：Congratulations! “"+text.bidder_name+"”the whole collection has been auctioned off</div>";
			}
          
		} else if (text.type == 4) {
			
			if(text.is_last_one==0){
	          	html += "<div class='bgRed'>presenter：Unfortunately, the collection has been streamed to the next collection</div>";
	          }else {
	          	html += "<div class='bgRed'>presenter：Unfortunately, the collection has been auctioned off.</div>";
	          }
		} else if (text.type == 6) {
			if(text.is_sale==0){
				html += "<div><pre><span class='col_red'>presenter：Welcome to《"+text.live_title+"》the appraisal live is about to begin</pre></div>";
			}else{
				html += "<div><pre><span class='col_red'>presenter：Welcome to《"+text.live_title+"》the auction is about to begin</pre></div>";
			}

		} else if (text.type == 7) {
			if(text.is_sale==0){
				html += "<div><pre><span class='col_red'>presenter：Thank you for your support, and the live ended</pre></div>";
			}else{
				html += "<div><pre><span class='col_red'>presenter：Thank you for your support. The auction ended</pre></div>";
			}
			setTimeout(function() {
				$(".player_ended").show();
			}, 5000)
			...
			html += "<div class='bgRed'>presenter：Welcome“" +text.user_name + "”to the live broadcast！</div>";
		} else if (text.type == 9) {
			html += "<div class='bgRed'>presenter：<pre>" + text.en_msg + "</pre></div>";
			changeAuctionGoods();

		} else if (text.type == 10) {
			$(".prod_others .watch_num").html(text.msg+"peoples online");
		} else if (text.type == 11) {
			html += "<div class='bgRed'>presenter：<pre>The anchor is back.</pre></div>";
		} else {
    var reply_user_name="";
			...
			if (text.is_presenter == "1") {
				html += "<div class='bgRed'><span class=''>presenter：</span>"+reply_user_name + text.msg + "</div>";
			} else {
...
function convertTextMsgToHtml2(content, num_type) {
	...
	if (text.type != undefined) {
      ...
      if(text.unsold_type=="1"){
				html+="<div class='bgRed'>Sorry that this collection does not reach reserve price. It has been an abortive auction.</div>";
			}else{
			    if(text.is_last_one==0){
		          	html += "<div class='bgRed'>presenter：Unfortunately, the collection has been streamed to the next collection</div>";
		          }else {
			...
			html += "<div class='bgRed'>presenter：<pre>" + text.msg + "</pre></div>";
		} else if (text.type == 10) {
