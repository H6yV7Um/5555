      ...
      for (var i = 0; i < collections.length; i++) {
        collHtml+='<li><a href="goodsDetail.html?id='+collections[i].id+'&mt=1">'+
           ...
           ' <div class="prices">'+
              ...
              '<input type="button" class="btn_tojoin" value="参与竞拍">'+
            '</div>'+
          '</a></li>'
      };
...
var getTypeList=function(){
  ...
  commonCla.ajaxCommonFun(url,"get",function(data,textStatus,request){
    if(textStatus=="success"){
       var menulist='<li class="cur" cid="0">全部</li>';
  ...
  commonCla.ajaxCommonFun(url,"get",function(data,textStatus,request){
    ...
    if(textStatus=="success"){
      ...
      for (var i = 0; i < paipin.length; i++) {
        var paipin_status='<span class="auc_status">直播中</span>';
        var p_status=paipin[i].status
        if(p_status==0){
           paipin_status='<span class="auc_status preView">待拍卖</span>';
          if(paipin[i].auctions!=undefined&& paipin[i].auctions!=null && paipin[i].auctions!="" && paipin[i].auctions.length>0){
            paipin_status='<span class="auc_status preView">开始时间：'+cutDate(paipin[i].auctions[0].begin_time,"ymd")+'</span>';
          }
        }else if(p_status==2){
          paipin_status='<span class="auc_status done">已成交</span>';
        }else if(p_status==3){
          paipin_status='<span class="auc_status lost">已流拍</span>';
        }
        ...
        paipin_html+='<li><a href="goodsDetail.html?id='+paipin[i].id+'&mt=1">'+
                      ...
                      '<div class="auc_desc">'+
                        '<p>'+paipin_name+'</p>'+
                          '<p class="">送拍机构：'+(user.name==undefined?"":user.name)+'</p>'+
                      '</div>'+
                    '</a></li>'
      };
      if(data.total==0){
...
var initGoodsDom=function(data){
  ...
  $(".priceName").html("当前价：");
  if(status==2){
    $(".priceName").html("成交价"+change_currency(data.currency));
  }else if(status==0){
    $(".goodsInfo h1").html("未开拍");
  }else if(status==3){
    $(".goodsInfo h1").html("");
  }
  ...
  $(".goods_seller").html("送拍机构："+data.user.name)
  ...
  //出价
  if(offers.length<=0){
    var offersHtml="<tr><td colspan='4' class='tc'>暂无出价记录</td></tr>"
  }else{
    var offersHtml=""
  }
  for (var i = 0; i < offers.length; i++) {
    ...
    if(i=="0"){
      if(data.status==1){
        status_label='<span class="status_leader">领先</span>';
      }else{
        status_label='<span class="status_leader">成交</span>';
      }
      cur="cur"
    }else{
      status_label='<span class="status_out">出局</span>'
    }
...
var initLiveList=function(data){
   ...
   for (var i = 0; i < lives.length; i++) {
    ...
    var live_status='<span class="auc_status">直播中</span>';
    var p_status=lives[i].status
    if(p_status==2){
        live_status='<span class="auc_status preView">精彩回放</span>';
    }
    var rolesHtml="";
        ...
        '<div class="auc_desc">'+
          '<p>'+name+'</p>'+
           ' <div class="btn_list">'+
             ...
             ' </div>'+
              '<div class="watch_num fr">观看'+lives[i].watch_num+'次</div>'+
            '</div>'+
        '</div>'+
...
var initDetailDom=function(data){
        ...
        '<div class="auc_desc">'+
          '<p>'+tit_name+'</p>'+
            '<div class="btn_list">'+
              ...
              '</div>'+
              '<div class="watch_num fr">观看'+mores[i].watch_num+'次</div>'+
            '</div>'+
        '</div>'+
  ...
  if(status==1){
    ...
    $("#experts_player").html('<img class="end_bg" src="'+data.cover+'!750x563"/><div class="end_tit">直播结束<div>')
  }
...
var initAuctionDom=function(data){
  ...
  $("#js_theme_desc .desc_con1").html('<p>送拍机构：'+data.user.name+'</p><p>开始时间：'+cutDate(data.begin_time,"ymd")+'</p>')
  $("#js_theme_desc .desc_con2").html('<p>专场介绍：</p><p class="desc_con">'+data.description.substr(0,100)+'</p>')
  var collections=data.collections;
  ...
  for (var i = 0; i < collections.length; i++) {
      ...
      if(p_status==0){
        paipin_status='<span class="auc_status preView">开始时间：'+cutDate(data.begin_time,"ymd")+'</span>';
      }else if(p_status==2){
        paipin_status='<span class="auc_status done">已成交</span>';
      }else if(p_status==3){
        paipin_status='<span class="auc_status lost">已流拍</span>';
    ...
    coll_html2+='<li><a href="goodsDetail.html?id='+collections[i].id+'&mt=1">'+
                ...
                '<div class="auc_desc">'+
                  '<p>'+collections[i].name+'</p>'+
                    '<p class="">送拍机构：'+collections[i].user.name+'</p>'+
                '</div>'+
              '</a></li>'
  };
      ...
      if (pageData.length > 0) {
       ...
       for (var i = 0; i < nLength; i++) {
        ...
        if (reply_data != null && reply_num>0) {
            ...
            var reply_num_html="";
            if(reply_num>=2){
              reply_num_html="<p class='reply_num js_reply_num'>查看全部"+reply_num+"条回复</p>"
            }else{
	     reply_num_html="<p class='reply_num'><p>"
            ...
            for (var a = 1; a < reply_data.length; a++) {
              more_replys+="<div class='reply_msg'>"+
                           "<span class='reply_name'>"+reply_data[a].user.name+"</span>回复<span class='reply_name'>："+reply_data[a].asker.name+"</span>"+
                           " <span>"+reply_data[a].content+"</span>"+
                           "<div class='cmt_time2'>" +
                              "<span class='cmt_data'>" + cutDate(reply_data[a].created_at)+ "</span>" +
                              "<span class='cmt_reply fr js_dialog'>回复</span>"+
                            "</div>" +
                         "</div>"
            };
            repliesHtml+="<div class='reply'>"+
                         "<div class='reply_msg'>"+
                           "<span class='reply_name'>"+reply_data[0].user.name+"</span>回复<span class='reply_name'>"+reply_data[0].asker.name+"：</span>"+
