         ...
         initAuction(ret,request);
          ...
          if(isWeiXin()){
            wx_share("[Auction]"+ret.name,
              ...
              "http://s.cangpai.lookmetv.com/en/auctions/auction.html?id="+commonCla.analyzParams("id"),
              ret.cover);
          }
        }else{
            if(ret.status==404){
...
var initAuction=function(ret,request){
    ...
    if(type==1){
   	...
   	var playHtml='<div class="player"></div><div class="deposit">deposit'+change_currency(ret.currency)+ret.deposit_buyer+'</div><div class="shadow"></div><div class="pre_times" ></div><div class="m-title">'+ret.name+'</div>'+
                '<div class="player_status" ></div>'
     $(".c-main .player_main").html(playHtml);
     $(".player-banner").attr("src",ret.cover);
	...
	var collects_inx='<span class="abs_inx">'+(i+1)+'</span>'
        if(c_status=="0"){
             ...
             doneHtml='<div class="status status_succ">'+
                        '<p>Starting：<span class="col_red price">'+change_currency(ret.currency)+collections[i].start_price+'</span></p>'+
                     '</div>';
              dongIcon='';  
        }else if(c_status=="1"){
            doneHtml='<p class="status status2 col_red">auction</p>'
            dongIcon='';
        }else if(c_status=="2"){
             doneHtml='<div class="status status_succ">'+
                        '<p>transaction price：<span class="col_red price">'+change_currency(ret.currency)+collections[i].amount+'</span></p>'+
                        '<p>bid winner:<span class="winner">'+changeName(collections[i].buyer)+'</span></p>'+
                     '</div>'
             dongIcon='<img src="assets/images/icon-done.png" class="icon_mark">'
        }else if(c_status=="3"){
            doneHtml='<p class="status status2 col_grey">auction over</p>';
    ...
    if(liveStatus==-1 || liveStatus==3){
        $(".pre_times").html(ret.begin_time+'start');
        $(".btn_ring").show();
        $(".btn_enter").html("Live not start");
    }else if(liveStatus==1){
       $(".pre_times").html('Living');
       $(".pre_times").css("color","#EB1010");
       
       $(".btn_enter").html("Enter the auction>");
       //$(".btn_point").show();
    }else{
        $(".m-title").css("bottom","25px");
        $(".btn_enter").css("color","#666");
        $(".btn_enter").html("auction over");
        //$(".btn_point").show();
    }
}
var initPage=function(){
    ...
    $(".btn_enter_main").click(function(){
        if($(this).find(".btn_enter").html().indexOf("Enter")!=-1){
      if($("#btn_point").attr("a_type")==4){
        window.location.href="../live2.0/share.html?type=1&id="+commonCla.analyzParams("id");
      }else{
         window.location.href="../live/share.html?type=1&id="+commonCla.analyzParams("id");
