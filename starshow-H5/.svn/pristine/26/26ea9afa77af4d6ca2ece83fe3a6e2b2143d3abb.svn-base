...
var initChartsData=function(start_time,end_time){
    var params={
        "start_time":start_time,
        "end_time":end_time,
        "star_id":commonCla.analyzParams("star_id"),
        "platform_id":commonCla.analyzParams("platform_id")
    }
  ...
  $(".da_header li").click(function(){
    if($(this).hasClass("cur")==false){
       $(".da_header li").removeClass("cur");
         ...
         initChartsData(before_time,end_time);
         if(num==7){
          counterN=3;
          counterS=0;
         }else{
  ...
  $(".btn-next").click(function(){
    ...
    if(!isCan){
      ...
      //获取数据
       ...
       initChartsData(first_time,after_time);
      if(counterS<=0){
