function initShareButtons() {
	...
	$('.hp-social-fb').attr('href', 'https://www.facebook.com/sharer/sharer.php?u=' + link);
	$('.hp-social-tw').attr('href', 'https://twitter.com/intent/tweet?text=' + text + '&url=' + link);
	$('.hp-social-gp').attr('href', 'https://plus.google.com/share?url=' + link);
}
...
function playM3U8(src) {
	...
	if ($('#player-proxy').prop('checked')) {
		playM3U8byGrindPlayer(src.replace(/https?:\/\//i, 'https://www.dianshibo.com/fetch.php/'));
	} else {
...
function playM3U8byGrindPlayer(src) {
	...
	swfobject.embedSWF('GrindPlayer.swf', 'player', '100%', '280', '10.2', null, flashvars, params, attributes);
}
