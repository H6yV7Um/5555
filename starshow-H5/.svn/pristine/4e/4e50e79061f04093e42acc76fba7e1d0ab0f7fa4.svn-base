   ...
   //活动分享
    var shareActivity=function(){
      setBridgeCallHandler(bridge, {
        'action': '3',
        'share': {
          'share_url':"https://lookmetv.com/starshow5.0/starZeroRange4/share.html",
          'title': "星秀福利社之杨紫签名福利",
          'content': "来时尚星秀APP，手动点评参与互动，赢取杨紫亲笔签名！",
          'cover': "https://starshow-pic.b0.upaiyun.com/default/111.png"
        }
      })
    }
})
  ...
  var initCommentPage=function(){
      ...
      if($("#btn_down").html()!=undefined){
	      $(".btn_msgSend").click(function(){
	        ...
	        if(jwt_token=="" || jwt_token==undefined){
            //授权
             window.location.href = host+"wechat/activityAuth?type=starZeroRange4";
         }else{
	          toComment(jwt_token);
	        }
	      })
