   ...
   //活动分享
    var shareActivity=function(){
      setBridgeCallHandler(bridge, {
        'action': '3',
        'share': {
          'share_url':"https://lookmetv.com/starshow5.0/starZeroRange3/share.html",
          'title': "“关关”乔欣的福利show",
          'content': "追剧有惊喜，互动拿福利，乔欣签名照等你来拿",
          'cover': "https://starshow-pic.b0.upaiyun.com/default/show/300.png"
        }
      })
    }
})
  ...
  var initCommentPage=function(){
      ...
      if($("#btn_down").html()!=undefined){
	      $(".btn_msgSend").click(function(){
	        ...
	        if(jwt_token=="" || jwt_token==undefined){
            //授权
             window.location.href = host+"wechat/activityAuth?type=starZeroRange3";
         }else{
	          toComment(jwt_token);
	        }
	      })
