...
var host="http://api.startvshow.com/v5"
var tcc={
...
var ceremonyMain={
   ...
   initRankCon:function(resultData,listPage,typeNum){
      ...
      for(var i=0;i<listDdata.length;i++){
        ...
        if(typeNum!=2){
          var a_url="<a href='hitList.html?star_id="+listDdata[i].uid+"&uid="+uid+"&"+new Date().toTimeString()+"'>";
        }else{
          var a_url="<a href='"+host+"share.html?action=jumpToPersonal&uid="+listDdata[i].uid+"'>";
        }
   ...
   navSwitch:function(typeNum){
    ...
    //获取数据
    $("#js-moreRank").hide();
    ceremonyMain.ajaxCommonFun(host+"festival/getlist?type="+Number(Number(typeNum)+1)
      +"&current_count="+listPage+"&uid=114655&server="+server+"&callback=?","get",function(resultData){
        //移除缓冲
     ...
     if(infoData.length<=0){
      $("#js-searchList").html("<p class='tc' style='margin-top:50px;font-size:1.5rem'>暂无此明星入驻</p>");
       ...
       for (var i = 0; i < infoData.length; i++) {
        ...
        searchHtml+="<a href='hitList.html?star_id="+infoData[i].uid+"&uid="+uid+"'><li class='curPersonInfo searchPersonInfo'>"
   ...
   toSearch:function(){
    ...
    var url=host+"festival/search?keyword="+keyword+"&server="+server+"&callback=?"
   ...
   initCereDetailHtml:function(resultData){
    ...
    if(resultData.info.hit_num>=3){
      $('#toHit').attr("disabled",true);
      $('#toHit').addClass("disabled");
      $("#toHit").attr("num",resultData.info.hit_num);
    }else{
    ...
    for(var i = 0; i < hitlist.length; i++) {
       ...
       if(i<=2){
        liHtml+="<a href='"+host+"share.html?action=jumpToPersonal&uid="+hitlist[i].uid+"'><li class='tops'>"
          +"<p class='head-pic'><img src='"+avatar+"'></p><p class='user-name'>"+uname+"</p>"
          +"<p class='rank-topsNum'><img src='assets/images/n"+Number(i+1)+".png' /></p>"
          +"<div class='follow-num'>"
          +"<span id='followNum'>"+likeNum+"</span>"
        ...
        liHtml+="<a href='"+host+"share.html?action=jumpToPersonal&uid="+hitlist[i].uid+"'><li>"
