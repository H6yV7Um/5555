...
var commonCla = {
		...
		//测试服务器
		/*hostBase: "http://testcangpai.startvshow.com/api/v4",
		webHostBase: "http://testcangpai.startvshow.com/api",*/
		//分享地址
		//ajax
		ajaxCommonFun: function(url, type, callbackFun, params={"language": en}) {
			$.ajax({
				url: url,
				type: type,
				dataType: 'json',
				async: true,
				cache: false,
				data: Object.assign({"language": en}, params)
				,
...
var timesReview=function(created_at,endTime){
      ...
      if(sTime<60){
      timeHtml=sTime+"s"+timeMark
      }else if(sTime>=60 && sTime<3600){
         /* timeHtml=Math.round(sTime/60)+"分钟"+sTime%60+"秒"+timeMark;*/
         timeHtml=Math.round(sTime/60)+"&nbsp;&nbsp;minutes"+timeMark;
      }else if(sTime>=3600 && sTime<3600*24){
          timeHtml=hDiff.toFixed(0)+"&nbsp;&nbsp;hours"+timeMark;
      }else if(sTime>=3600*24 && sTime<3600*24*30){
          timeHtml=dayDiff+"&nbsp;&nbsp;days"+timeMark;
      }else if(sTime>=3600*24*30 && sTime<3600*24*30*12){
      	  timeHtml=(dayDiff/30).toFixed(0)+"&nbsp;&nbsp;months"+timeMark;
      }else{
      	 timeHtml=(dayDiff/30/12).toFixed(0)+"&nbsp;&nbsp;years"+timeMark;
      }
