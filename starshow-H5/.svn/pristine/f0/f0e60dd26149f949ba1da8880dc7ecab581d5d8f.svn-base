...
var live_share=function(ret){
	...
	if(isWeiXin()){
	  if(link_type==1){
	  	...
	  	shareurl="http://s.cangpai.lookmetv.com/live2.0/share.html?type=1&id="+commonCla.analyzParams("id")


	  }else{
	  	title="[藏拍专家]"+ret.user.name+"的鉴宝直播";
	  	content=ret.name;
	  	shareurl="http://s.cangpai.lookmetv.com/live2.0/share.html?id="+commonCla.analyzParams("id")
	  }
	  cover=ret.cover
...
var initLive=function(ret,request){
	...
	$("title").html(ret.name);
	$(".watch_num").html(commonCla.cWan(ret.watch_num)+"人在线");
