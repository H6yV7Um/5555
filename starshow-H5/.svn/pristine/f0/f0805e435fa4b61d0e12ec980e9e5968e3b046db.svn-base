...
window.___E_mod(function (E, $) {
	...
	E.fn.addTxt = function () {
		...
		var $txt = $(
			'<div contentEditable="true" class="wangEditor-mobile-txt" id="wangEditor-mobile-txt">' + 
			val + 
			'</div>'
		);
...
window.___E_mod(function (E, $) {
	...
	E.fn.addMenuCheck = function (menuId) {
		...
		menus[menuId] = {
			...
			// 触发器
			$trigger: $('<div><i class="icon-wangEditor-m-ellipsis-h"></i></div>'),
			// 包裹触发器的容器
			$wrap: $('<div class="item"></div>'),
			...
			bindEvent: function (editor) {
				...
				menuData.$trigger.on('singleTap', function (e) {
					...
					// 初始化 点击事件 事件
					//添加链接
					$(".btn_addLink").on('singleTap', function (e) {
						...
						var appendHtml = "<a href='" + link.val() + "'>" + linkcon.val() + "</a>";
						// 构建dom结构
						var $link = $(appendHtml);
	...
	E.fn.addMenuImg = function (menuId) {
		...
		menus[menuId] = {
			...
			bindEvent: function (editor) {
				...
				//测试 
				//imgOnload(window.URL.createObjectURL(file))
				//imgOnloadError();

				//del input 有文件选中时，显示预览图，提交 form
				...
				$trigger.on('singleTap', function (e) {
					...
					function fn() {
						 // ---------- 调用客户端上传 ----------
						//window.editorRequest.uploadImage(wangEditorMobileFile);
						window.editorRequest.getContentPic()
						return false;
						//test
						//imgOnOpen("assets/images/cover.png");
...
//adddddddd
var prveImgContainerId = '';
var prevImgInfoId ='';
var imgSelf='';
var imgOnload = function (resultSrc) {
...
var imgOnOpen=function(prevImgSrc){
	var $focusElem = imgSelf.$focusElem;
	prveImgContainerId = 'div' + Math.random().toString().slice(2);
	prevImgInfoId = 'info' +  Math.random().toString().slice(2);
	// 生成预览图片，设置半透明
	$focusElem.after(
