...
var getNewsData=function(){
	var url=host+"goods/"+new_id+"/news?jwt_token="+jwt_token;
	commonCla.ajaxCommonFun(url,"get",function(ret){
      ...
      if(ret.code=="200"){
        initNewsDom(ret);
        //二次分享
        var shareurl=commonCla.shareUrlBase+"/starshow5.0/shop/news/share.html?new_id="+commonCla.analyzParams("new_id");
        wx_share(ret.data.title,"时尚星秀-全明星时尚平台，让你更时尚，让你更闪耀！",shareurl,ret.data.cover);
...
var changeBuyNum=function(idNum,content){
  //请求数量  
  var url=host+"goods/"+idNum+"/salesNum";
  var sales_num=0;
  commonCla.ajaxCommonFun(url,"get",function(ret){
   if(ret.code=="200"){
   sales_num=ret.data.sales_num;
   }
    $(content).find("#prod_"+idNum).find("div.prod_tit").eq(1).find("p").html(commonCla.cWan(sales_num)+"个小伙伴已经入手！你还等什么？")
  })
...
var initNewsDom=function(data){
   ...
   var newsHtml='<section class="news_title">'+
		...
		'<div class="mt10 channel_info">'+
			...
			'<div></div>'+
		'<div class="tl w90_persent col_grey"><span id="curTime">'+timesReview(data.data.created_at,data.current_time)+'</span>'+
    '<span id="watch_num" >'+commonCla.cWan(data.data.watch_num)+'</span>人浏览'+
    '<span class="" id="like_num2" >'+commonCla.cWan(data.data.like_num)+'</span>人喜欢 </div>'+
		'</div>'+
		'<!--内容-->'+
    ...
    //自己的商品
    var prodConSelf=$(".newContent").find(".prodConSelf");
    for (var i = 0; i < prodConSelf.length; i++) {
      var id=$(prodConSelf[i]).attr("itemId")
      changeBuyNum(id,$(".newContent"));
    };
        ...
        if(isWeiXin()){
          if(jwt_token=="" || jwt_token==undefined){
            ...
            if(sourceType=="reply"){
              ...
              wx_authorize(params,"goodsNewsComment");
            }else{
              var params={
                "id":id,
              }
              wx_authorize(params,"goodsNews");
            }
...
var toComment = function(content) {
  ...
  var url = host + "goods/news/" + id + "/comment";
...
var toReply=function(content){
  ...
  var url = host + "goods/news/" + cid + "/reply";
  commonCla.ajaxCommonFun(url, "post", function(resultData) {
      $(".btn_msgSend").removeClass("btn_loadding");
    $(".btn_msgSend").val("发表");
    $(".btn_msgSend").removeAttr("disabled");
...
var getCommentData=function(){
  ...
  var url=host+"goods/"+news_id+"/news/comment";
  var params={
    "count":5
  }
...
var tapLike=function(jwt_token){
      ...
      var url=host+"goods/news/"+id+"/like?jwt_token="+jwt_token;
...
var getReplayData=function(){
  ...
  var url=host+"goods/"+cid+"/news/reply?current_count="+current_count;
  commonCla.ajaxCommonFun(url,"get",function(ret){
    $(".postMain").show();
    $(".loadding").hide();
    $(".news_nav").show();
