...
var reviewContent=function(data){
    ...
    $("#share_info").attr("title","[藏拍资讯]"+data.title);
    $("#share_info").attr("cover",data.cover+"!250x250");
    //$("#share_info").attr("shareUrl","http://testshare.startvshow.com/collectionAuction/news/share.html?id="+new_id);
   ...
   var newsHtml=
    ...
    '</div>'+
        ...
        '<span class="n-times">'+create_time+'</span></div>'+
			'</div>'+
      '<div userId="'+user.id+'" class="btn_concern">关注</div>'+
		'</div>'+
    ...
    '<h3 class="labelTit '+label_isBlock+'">相关标签：</h3>'+
		...
		'<!--<div class="tl w90_persent col_grey">浏览 <span class="watch_num" >'+commonCla.cWan(data.watch_num)+
    '</span><span class="ml10 watch_num" >喜欢 '+commonCla.cWan(data.like_num)+'</span></div>-->'
