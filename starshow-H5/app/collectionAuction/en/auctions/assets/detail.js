...
var initDetailDom=function(ret,request){
	...
	var freight_html="";
  if(ret.take_type==1){
	  freight_html='<li>Freight:<span class="start_freight">'+change_currency(ret.currency)+ret.freight+'</span></li>'
	}
  var gz="";
		if(ret.estimate!="" && ret.estimate!=undefined){
			gz='<li>start_bail：<span class="start_price">'+change_currency(ret.currency)+ret.estimate+'</span></li>'
		}
	var bail="";
	...
	if(status==0){
		...
		if(ret.auctions!=null && ret.auctions.length>0){
			...
			$(".pre_times2").html(str_time+"start");
			$(".pre_times2").show();
		}
		//简介
		...
		good_desc_point='<ul>'+
	    		'<li>starting price:<span class="start_price">'+change_currency(ret.currency)+ret.start_price+'</span></li>'+
	    		'<li>deposit：<span class="start_bail">￥'+bail+'</span></li></ul>'
	    good_desc='<ul>'+
	    		'<!--<li>starting price:<span class="start_price">'+change_currency(ret.currency)+ret.start_price+'</span></li>'+
	    		'<li>deposit：<span class="start_bail">￥'+bail+'</span></li>-->'+
	    		'<!--<li>Hedge price:<span class="start_bail">'+change_currency(ret.currency)+ret.insurance+'</span></li>-->'+
	    		'<li>reserve price：<span class="start_retainage">'+(ret.retainage==0?"no":"yes")+'</span></li>'+
	    		gz+freight_html+
	    	    '</ul>'
		
		$(".btn_enter").html("Pay Bond");
		$(".btn_enter").addClass("js_btn_down");
		$(".btn_enter").removeClass("toLive");
	}else if(status==1 || status==3){
		var currentPrice=""
		if(status==1){
	    	currentPrice='<li class="w100">current price:<span class="current_price">'+change_currency(ret.currency)+ret.start_price+'</span></li>'
	    }
	    ...
	    if(status==1){
	    	if(!$(".btn_enter").hasClass("toLive")){
	    		...
	    		$(".btn_enter").html("into the auction&gt;");
	    		$(".btn_enter").attr("status","1");
	    		//$(".btn_point").show();
	    	}
	    	$(".pre_times2").html("Auction");
		$(".pre_times2").show();
		if(ret.auctions[0].type==4){
			$(".pre_times2").html('From the end<p id="countdown" style="display:inline"></p><div class="skew"></div>');
			...
			var footersHtml='<a href="../protocol/sevenDayHelp.html">'+
					    		...
					    		'<span class="vm">Daily auction rules</span></a>'
      ...
      var bTime=timeFormat(new Date(request.getResponseHeader("Date")));
			...
			$("#countdown").html('<span>'+time.d+'</span>day<span>'+time.h+'</span>hours<span>'+time.m+'</span>s')
	    	...
	    	good_desc_point='<ul>'+
	    		'<li class="w100_normal">starting price:<span class="start_price">'+change_currency(ret.currency)+ret.start_price+'</span></li></ul>'
     		good_desc='<ul>'+
	    		'<!--<li>starting price:<span class="start_price">'+change_currency(ret.currency)+ret.start_price+'</span></li>-->'+
	    		'<li>reserve price：<span class="start_retainage">'+(ret.retainage==0?"no":"yes")+'</span></li>'+
	    	    '</ul>'
	    ...
	    good_desc='<ul>'+
	    		'<li>transaction price:<span class="start_price">'+change_currency(ret.currency)+ret.amount+'</span></li>'+
	    		'<!--<li>Hedge price:<span class="start_bail">'+change_currency(ret.currency)+commonCla.cWan(ret.insurance)+'</span></li>-->'+
	    		'<li>reserve price：<span class="start_retainage">'+(ret.retainage==0?"no":"yes")+'</span></li>'+
	    		'<li>bid winner:<span class="start_rise">'+(ret.buyer==null?"anonymous":changeName(ret.buyer))+'</span></li>'+
	    	    '</ul>'
	    $(".btn_enter").addClass("btn_download");
	    $(".btn_enter").addClass("disabled");
	    $(".btn_enter").removeClass("toLive");
	    $(".btn_enter").html("finished");
  ...
  $(".goodsInfo_point").html(good_desc_point);
	...
	if(appraisals.length>0){
		...
		$(".expertList .fl span").html("<a href='expertsMsg.html?cid="+commonCla.analyzParams("id")+"'>Without experts to identify, write the first one</a>")
	}
	//拍卖参数
	...
	commonCla.ajaxCommonFun(url,"get",function(ret,textStatus,request){
		...
		if(textStatus=="success"){
		 	if(ret.auctions[0].is_check=="2"){
			    $(".c-main").html('<div class="noData"><img src="assets/images/weigui.png"/><p>The product has been removed due to violations</p></div>');
			}else{
		 ...
		 getPriceData();
		    ...
		    */
		    ...
		    //二次分享
	      if(isWeiXin()){
	        wx_share("[Auction]"+ret.name,
	          "[CANGPAI]A trading platform focused on collectibles！",
	          "http://s.cangpai.lookmetv.com/en/auctions/goodDetail.html?id="+commonCla.analyzParams("id")+"&auctionId="+commonCla.analyzParams("auctionId"),
	          ret.cover);
	      }
		}else{
			if(ret.status==404){
...
var initPriceDom=function(ret,request){
	...
	if($("#priceList li").length+15>=ret.total){
		$("#price_more").html("no more")
	}
	if(ret.page_data.length<=0 && $("#priceList li").length<=0){
		html='<div class="tc noprice">There is no record</span>'
	}else{
		
		for (var i = 0; i < ret.page_data.length; i++) {
			if(i==0 && $("#priceList li").length<=0){
				var icon_mark="bid";
				if($(".btn_enter").attr("status")=="1" || $(".btn_enter").attr("")=="abortive"){icon_mark="Lead";}
				
				html+='<li><span class="icon_lead">'+icon_mark+'</span><span>'+changeName(ret.page_data[i].user.name)+
				'</span>bid'+change_currency(ret.currency)+'<span class="price">'+ret.page_data[i].price+'</span></li>'
			}else{
				html+='<li><span class="icon_lead">out</span><span>'+changeName(ret.page_data[i].user.name)+
				'</span>bid'+change_currency(ret.currency)+'<span class="price">'+ret.page_data[i].price+'</span></li>'
			}
		}
...
var initCollection=function(ret){
  ...
  var collects= '<div class="fl"><img src="'+ret.collection.cover+'!300x225" /></div>'+
			  	'<div class="fr">'+
			  		...
			  		'<p class="price">Starting:'+change_currency(ret.currency)+ret.collection.start_price+'</p>'+
			  		'<div class="experts">A total of <span class="col_red">'+expertsNum+'</span>master has been identified</div>'+
			  	'</div>'
