	...
	//普通直播
		...
		if(re_lives.length>0){
			for (var i = 0; i < re_lives.length; i++) {
				...
				if(status==1){
					label_status='<span class="label_status living">Living</span>'
				}else if(status==2){
					label_status='<span class="label_status replay">Replay</span>'
				}else{
					...
					label_status='<span class="label_status preview">'+str_time+'start</span>'
				}
