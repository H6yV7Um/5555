...
function convertTextMsgToHtml(content, num_type) {
	...
	if (text.type != undefined) {
		...
		if (text.type == 0) {
			...
			html += "<div><span class='col_red'>" + text.user_name + "：</span><span class='col_red'>"+$(".js_initPrice").html().substr(0,1) + text.msg + "</span></div>";
		} else if (text.type == 1 || text.type == 3) {
			html += "<div class='bgRed'>presenter：" + text.msg + "</div>";
		} else if (text.type == 2) {

		} else if (text.type == 4) {
			html += "<div class='bgRed'>presenter：" + text.msg + "</div>";
			/*if(){
				html += "<div><pre><span class='col_red'>presenter：</span>很遗憾商品已流拍，全部藏品已经拍卖完毕</pre></div>";

			}else{
				html += "<div><pre><span class='col_red'>presenter：</span>很遗憾商品已流拍，将进入下一个商品拍卖</pre></div>";
			}*/
			//changeAuctionGoods();
		} else if (text.type == 6) {
			html += "<div><pre><span class='col_red'>presenter：</span>Welcome to CANGPAI, the auction is about to begin</pre></div>";

		} else if (text.type == 7) {
			html += "<div><pre><span class='col_red'>presenter：</span>Thank you for your support. The auction will end.</pre></div>";
			setTimeout(function() {
				$(".player_ended").show();
			}, 5000)
			...
			$(".prod_others .watch_num").html(text.msg+"online");
			/*$("#user-icon-fans").html(text.msg);
			html += "<div>系统消息：现在有" + text.msg + "人在线</div>";*/
		} else {
			if (text.is_presenter == "1") {
				html += "<div class='bgRed'><pre><span class='col_red'>presenter：</span>" + text.msg + "</pre></div>";
			} else {
