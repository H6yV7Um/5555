    ...
    $(".btn_msgSend").click(function(e){
      ...
      if (token == "" || token == undefined) {
        ...
        if (content.trim().length>100) {
          swal({
              "title":"Comments can not exceed 100 words",
              "confirmButtonText":"ok",
              "confirmButtonColor": "#ff1d3e",
              "animation":"none",
...
var commentFun=function(){
  ...
  $(".cmt_list").on("click", ".cmt_reply", function() {
      ...
      $(".txtMsg").attr("placeHolder","Respond " + cname + "：");
   ...
   //点击遮罩层删除回复
    $("#msg_overlay").click(function() {
        if ($(this).css("display") != "none") {
          ...
          $(".txtMsg").attr("placeHolder","Post a comment...");
          $(".txtMsg").val("");
          $(".txtMsg").focus();
        }
      })
    ...
    if(textStatus=="success"){
      ...
      if (pageData.length > 0) {
       ...
       for (var i = 0; i < nLength; i++) {
        ...
        if (pageData[i].replies != null) {
            ...
            var reply_num_html="";
            if(reply_num>=2){
              reply_num_html="<p class='reply_num'>View "+reply_num+"&nbsp;&nbsp;Respond</p>"
            }
      ...
      if(listPage>0){
        $(".btn_more").html("no more");
         ...
         $(".cmt_list").html('<div class="tc mt30 pb30"><h1 style="font-size: 1.5rem;">no comment</h1><p class="mt20">Come on to post the first comment ~</p></div>')
         if($("#hide_source").attr("iShare")!="true"){
           $(".btn_more").hide();
         }else{
          $(".btn_more").html("<p class='js_btn_down'>Download／Open APP</p>");
         }
...
//时间
  ...
  if (pageData.length > 0) {
   ...
   for (var i = 0; i < pageData.length; i++) {
    ...
    liHtml += "<li uid='" + pageData[i].user.id + "'uname='" + pageData[i].user.name + "' cid='" + pageData[i].id + "'>" +
      ...
      "</div>" +
      "<div class='cmt_content'>" +
      "<p class='cmt_comment cmt_reply'><span class='col_grey'>Respond<span class='reply_name'>"+pageData[i].asker.name+"：</span></span>" + pageData[i].content +
      "<!--<p class='cmt_reply'>Respond</p>-->"+
      " </p>"
      "</div>" +
      "</li>"
  ...
  if(current_count<15){
    ...
    if(current_count<data.total){
      $("#btn_more").show();
      $("#btn_more").html("<p>Click to load more</p>");
    }else{
      $("#btn_more").hide();
    }
  ...
  commonCla.ajaxCommonFun(url,"get",function(ret,textStatus,request){
    ...
    if(textStatus=="success"){
      ...
      if( iShare=commonCla.analyzParams("ishare")=="true"){
       $("title").html(ret.total+"Respond")
      }else{
         $("title").html(ret.total+"Respond&0&0")
      }
