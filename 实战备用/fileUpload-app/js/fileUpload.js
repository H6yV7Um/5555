...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        $("#"+uploadId).find(".box").append(btsStr);
    },
    /**
     * 初始化拖拽事件
     * @param opt 参数对象
    ...
    "addFileList":function(fileList,opt){
        ...
        var addJsObj =  $("#"+uploadId+" .box"+" .uploadBts").get(0);
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                if(imgtest.test(fileList[i].type)){
                    ...
                    $(fileModel).insertBefore(addJsObj);
                }else{
    ...
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId){
        ...
        // modelStr+=" </div>";
        modelStr+= `<div class="fileItem" filecodeid=${fileCodeId}>
                        <div class="imgShow">${showTypeStr} </div>
                        <div class="status">
    ...
    "uploadFile":function(opt){
        ...
        // 判断有没有自定义属性 有的话就遍历添加到formData里
        if(opt.otherData!=null&&opt.otherData!=""){
            for(var j=0;j<opt.otherData.length;j++){
                for( var k in opt.otherData[j]) {
                    formData.append(k,opt.otherData[j][k]);
                }
            console.log(k,opt.otherData[j][k])
            }
        }
        ...
        for(var i=0;i<fileList.length;i++){
            if(fileList[i]!=null){
                formData.append("UploadName",fileList[i]);
            }
        }
...
var uploadEvent = {
    ...
    "selectFileEvent":function(opt){
        ...
        inputObj.setAttribute("name",'UploadName');
